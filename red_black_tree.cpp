...
#define right children[RIGHT]
  ...
  leaf_type *find_rec(int key, leaf_type *ptr) {
    if (ptr->key == key)
      return ptr;
    if (ptr->key > key) {
      if (ptr->children[0]->_sentinel)
  ...
  bool insert(int key, std::string val) {
    ...
    if (!size) {
      root = new leaf_type(key, val, sentinel, sentinel);
      root->color = BLACK;
      size++;
      return true;
    }
  ...
  int insert_rec(int key, std::string val) {
    ...
    if (key < cursor->key) {
      if (cursor->children[0]->_sentinel)
        cursor->children[0] = new leaf_type(key, val, cursor, sentinel);
      else {
      ...
      if (cursor->children[1]->_sentinel)
        cursor->children[1] = new leaf_type(key, val, cursor, sentinel);
      else {
